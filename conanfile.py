...
class PicasoXCoreConan(ConanFile):
    name = "picasoxcore"
    license = "AGPL-3.0"
    author = "PICASO 3D"
    url = "https://github.com/PICASO3D/PicasoXCore"
    description = "Powerful, fast and robust engine for converting 3D models into g-code instructions for 3D printers. Based on opensource project CuraEngine."
    topics = ("picasoxcore", "protobuf", "gcode", "c++", "curaengine", "libarcus", "gcode-generation", "3D-printing")
    ...
    def layout(self):
        ...
        self.cpp.package.libs = ["_PicasoXCore"]
    ...
    def package(self):
        ...
        packager.run()
        copy(self, "PicasoXCore*", src = self.build_folder, dst = path.join(self.package_folder, "bin"))
        copy(self, "LICENSE*", src = self.source_folder, dst = path.join(self.package_folder, "license"))

    def package_info(self):
        ext = ".exe" if self.settings.os == "Windows" else ""
        if self.in_local_cache:
            self.conf_info.define("user.picasoxcore:picasoxcore", path.join(self.package_folder, "bin", f"PicasoXCore{ext}"))
        else:
            self.conf_info.define("user.picasoxcore:picasoxcore", path.join(self.build_folder, f"PicasoXCore{ext}"))
