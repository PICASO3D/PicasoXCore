...
void Application::printHelp() const
{
    logAlways("\n");
    logAlways("usage:\n");
    logAlways("PicasoXCore help\n");
    logAlways("\tShow this help message\n");
    logAlways("\n");
#ifdef ARCUS
    logAlways("PicasoXCore connect <host>[:<port>] [-j <settings.def.json>]\n");
    ...
    logAlways("\n");
#endif //ARCUS
    logAlways("PicasoXCore slice [-v] [-p] [-j <settings.json>] [-s <settingkey>=<value>] [-g] [-e<extruder_nr>] [-o <output.gcode>] [-l <model.stl>] [--next]\n");
    logAlways("  -v\n\tIncrease the verbose level (show log messages).\n");
#ifdef _OPENMP
    ...
    logAlways("PicasoXCore slice [general settings] \n\t-g [current group settings] \n\t-e0 [extruder train 0 settings] \n\t-l obj_inheriting_from_last_extruder_train.stl [object settings] \n\t--next [next group settings]\n\t... etc.\n");
    logAlways("\n");
    logAlways("In order to load machine definitions from custom locations, you need to create the environment variable PICASO_XCORE_SEARCH_PATH, which should contain all search paths delimited by a (semi-)colon.\n");
    logAlways("\n");
}
...
void Application::printLicense() const
{
    ...
    logAlways("Copyright (c) 2022 PICASO 3D\n");
    logAlways("\n");
    ...
    logAlways("along with this program.  If not, see <https://www.gnu.org/licenses/>.\n");
}
