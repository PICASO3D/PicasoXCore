...
void LayerPlan::addWall_p0(ConstPolygonRef wall, int start_idx, const WallSeamConfig& seamConfig, const SliceMeshStorage& mesh, const PathConfigStorage::MeshPathConfigs& mesh_config, WallOverlapComputation* wall_overlap_computation, coord_t wall_0_wipe_dist, float flow_ratio, bool always_retract)
{
	...
	start_idx = locateFirstSupportedVertex(wall, start_idx);
	const bool unchanged_supported_vertex = start_idx == tmp_start_idx;
	...
	if (unchanged_supported_vertex)
	{
		...
		{
			const Point& ps0 = seamConfig.point_center;
			const Point& ps1 = p0;

			if (on_seam_point == false)
			{
				addTravel(ps0, wall_min_flow_retract);
			}

			const float flow = (wall_overlap_computation) ? flow_ratio * wall_overlap_computation->getFlow(ps0, ps1) : flow_ratio;

			if (flow >= wall_min_flow)
			{
				first_line = false;
				addWallLine(ps0, ps1, mesh, non_bridge_config, bridge_config, flow, non_bridge_line_volume, speed_factor, distance_to_bridge_start);
			}
			else
			{
				travel_required = true;
	...
	for (size_t point_idx = 1; point_idx < wall.size(); point_idx++)
	{
		const Point& p1 = wall[(start_idx + point_idx) % wall.size()];
		const float flow = (wall_overlap_computation) ? flow_ratio * wall_overlap_computation->getFlow(p0, p1) : flow_ratio;
...
void LayerPlan::addWalls_p0(const Polygons& walls, const SliceMeshStorage& mesh, const PathConfigStorage::MeshPathConfigs& mesh_config, WallOverlapComputation* wall_overlap_computation, const ZSeamConfig& z_seam_config, coord_t wall_0_wipe_dist, float flow_ratio, bool always_retract)
{
	...
	const EZSeamCross z_seam_cross = mesh.settings.get<EZSeamCross>("z_seam_cross");
	const float z_seam_cross_start_flow = static_cast<float>(mesh.settings.get<Ratio>("z_seam_cross_start_flow"));
	const float z_seam_cross_start_speed = static_cast<float>(mesh.settings.get<Ratio>("z_seam_cross_start_speed"));
	const coord_t z_seam_cross_start_distance = mesh.settings.get<coord_t>("z_seam_cross_start_distance");
	const float z_seam_cross_finish_flow = static_cast<float>(mesh.settings.get<Ratio>("z_seam_cross_finish_flow"));
	const float z_seam_cross_finish_speed = static_cast<float>(mesh.settings.get<Ratio>("z_seam_cross_finish_speed"));
	const coord_t z_seam_cross_finish_distance = mesh.settings.get<coord_t>("z_seam_cross_finish_distance");
	...
	for (unsigned int poly_idx : orderOptimizer.polyOrder)
	{
		...
		{
			for (size_t ind = 0; ind < wall.size(); ind++)
			{
				const Point& p1 = wall[ind];
		...
		Point p1 = wall[start_idx]; // center seam
		ZSeamCrossing seamCrossing(wall, start_idx);
		if (z_seam_config.corner_pref == EZSeamCornerPrefType::Z_SEAM_CORNER_PREF_SMOOTH)
		{
			p1 = orderOptimizer.polyPointValue[poly_idx];
			seamCrossing = ZSeamCrossing(wall, start_idx, p1);
		}
		...
		{
			Point p0; // start & end
			if (seamCrossing.FindCrossing_Mode_Normal(walls, p0, z_seam_cross_start_distance, z_seam_cross_finish_distance, true))
			{
		...
		{
			const float angle_divider = (z_seam_cross == EZSeamCross::HALF) ? 2.0 : 4.0;
			Point p0_start;
			Point p0_end;
			if (seamCrossing.FindCrossing_Mode_Custom(walls, p0_start, p0_end, z_seam_cross_start_distance, z_seam_cross_finish_distance, angle_divider, true))
			{
...
void LayerPlan::writeGCode(GCodeExport& gcode)
{
	...
	if (gcode.getFlavor() == EGCodeFlavor::PICASO && gcode.getExtrudersUsed() < 2)
	{
    ...
    for(size_t extruder_plan_idx = 0; extruder_plan_idx < extruder_plans.size(); extruder_plan_idx++)
    {
        ...
        for(unsigned int path_idx = 0; path_idx < paths.size(); path_idx++)
        {
            ...
            if (!path.config->isTravelPath() && last_extrusion_config != path.config)
            {
                ...
                {
                    std::ostringstream feature_string;
                    feature_string << "M1105 S" << (int)path.config->feature;

                    switch (path.config->feature)
                    {
                    case PathConfigFeature::Inset0: feature_string << " ;Inset0"; break;
					case PathConfigFeature::Inset1: feature_string << " ;Inset1"; break;
                    case PathConfigFeature::InsetX: feature_string << " ;InsetX"; break;

                    case PathConfigFeature::BridgeInset0: feature_string << " ;BridgeInset0"; break;
					case PathConfigFeature::BridgeInset1: feature_string << " ;BridgeInset1"; break;
                    case PathConfigFeature::BridgeInsetX: feature_string << " ;BridgeInsetX"; break;
                    case PathConfigFeature::BridgeSkin1: feature_string << " ;BridgeSkin1"; break;
                    case PathConfigFeature::BridgeSkin2: feature_string << " ;BridgeSkin2"; break;
                    case PathConfigFeature::BridgeSkin3: feature_string << " ;BridgeSkin3"; break;
                    ...
                    case PathConfigFeature::RaftBase: feature_string << " ;RaftBase"; break;
                    case PathConfigFeature::RaftInterface: feature_string << " ;RaftInterface"; break;
                    case PathConfigFeature::RaftSurface: feature_string << " ;RaftSurface"; break;

                    case PathConfigFeature::ExtruderTravel: feature_string << " ;ExtruderTravel"; break;
                    case PathConfigFeature::ExtruderSkirtBrim: feature_string << " ;ExtruderSkirtBrim"; break;
                    case PathConfigFeature::ExtruderPrimeTower: feature_string << " ;ExtruderPrimeTower"; break;

                    case PathConfigFeature::SupportRoof: feature_string << " ;SupportRoof"; break;
                    case PathConfigFeature::SupportInfill: feature_string << " ;SupportInfill"; break;
                    case PathConfigFeature::SupportBottom: feature_string << " ;SupportBottom"; break;
                    case PathConfigFeature::SupportUnderRoof: feature_string << " ;SupportUnderRoof"; break;
                    case PathConfigFeature::SupportAboveBottom: feature_string << " ;SupportAboveBottom"; break;
                    }

                    gcode.writeCode(feature_string.str().c_str());
                }
            ...
            { // early comp for travel paths, which are handled more simply
                ...
                {
                    CheckAdditionalCommentsGCode(gcode, layer_nr, path.points[point_idx], true);
            ...
            if (!spiralize) // normal (extrusion) move (with coasting
            {
                ...
                { // normal path to gcode algorithm
					...
					// <SupportInterfaceInjection>
					coord_t z_support_interface_offset = 0;
					if (layer_nr > 0)
					{
						const coord_t support_z_roof_offset = extruder.settings.get<coord_t>("support_z_roof_offset");
						const coord_t support_z_bottom_offset = extruder.settings.get<coord_t>("support_z_bottom_offset");

						switch (path.config->feature)
						{
						case PathConfigFeature::SupportRoof:
							z_support_interface_offset = -support_z_roof_offset;
							break;

						case PathConfigFeature::SupportBottom:
							z_support_interface_offset = support_z_bottom_offset;
							break;

						default: z_support_interface_offset = 0; break;
						}
					}
					gcode.setZ(z + z_support_interface_offset);
					// </SupportInterfaceInjection>
                    ...
                    for(unsigned int point_idx = 0; point_idx < path.points.size(); point_idx++)
                    {
                        ...
                        CheckAdditionalCommentsGCode(gcode, layer_nr, path.points[point_idx]);
                        gcode.writeExtrusion(path.points[point_idx], speed, path.getExtrusionMM3perMM(), path.config->type, path.config->feature, update_extrusion_offset);
                    }
                }
            }
            else
...
void LayerPlan::CheckAdditionalCommentsGCode(GCodeExport& gcode, size_t layer_nr, Point& dest_point, bool move)
{
    bool overhang_state = false;
    bool model_on_support_state = false;

    if (!move)
    {
        const Point current = gcode.getPositionXY();
		if (gcode.getFlavor() == EGCodeFlavor::PICASO)
        {
            for (const SliceMeshStorage& mesh : storage.meshes)
            {
                if (overhang_state == false
                    && mesh.unsupported_areas.size() > layer_nr
                    && (mesh.unsupported_areas[layer_nr].inside(current) || mesh.unsupported_areas[layer_nr].inside(dest_point)))
                {
                    overhang_state = true;
                }

                if (model_on_support_state == false
                    && mesh.model_on_support_areas.size() > layer_nr
                    && (mesh.model_on_support_areas[layer_nr].inside(current) || mesh.model_on_support_areas[layer_nr].inside(dest_point)))
                {
                    model_on_support_state = true;
                }
            }
        }
    }

    if (gcode.getLastOverhangState() != overhang_state)
    {
        std::ostringstream tmp;
        if (overhang_state) {
            tmp << "M1103 ;OVERHANG:START";
        }
        else {
            tmp << "M1104 ;OVERHANG:END";
        }
        gcode.writeCode(tmp.str().c_str());
        gcode.setLastOverhangState(overhang_state);
    }
    if (gcode.getLastModelOnSupportState() != model_on_support_state)
    {
        std::ostringstream tmp;
        if (model_on_support_state) {
            tmp << "M1101 ;MODEL_ON_SUPPORT:START";
        }
        else {
            tmp << "M1102 ;MODEL_ON_SUPPORT:END";
        }
        gcode.writeCode(tmp.str().c_str());
        gcode.setLastModelOnSupportState(model_on_support_state);
    }
}
...
bool LayerPlan::writePathWithCoasting(GCodeExport& gcode, const size_t extruder_plan_idx, const size_t path_idx, const coord_t layer_thickness)
{
    ...
    { // write normal extrude path:
        ...
        for(size_t point_idx = 0; point_idx <= point_idx_before_start; point_idx++)
        {
            ...
            gcode.writeExtrusion(path.points[point_idx], extrude_speed, path.getExtrusionMM3perMM(), path.config->type, path.config->feature);
        }
        ...
        gcode.writeExtrusion(start, extrude_speed, path.getExtrusionMM3perMM(), path.config->type, path.config->feature);
    }
