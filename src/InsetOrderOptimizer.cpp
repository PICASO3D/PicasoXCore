    ...
    if (!outer_inset_first && mesh.settings.get<bool>("infill_before_walls"))
    {
        ...
        for (unsigned inset_level = num_insets - 1; inset_level > 0; --inset_level)
        {
            ...
            if (insets_that_do_not_surround_holes.size() > 0 && extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_x_extruder_nr").extruder_nr)
            {
                ...
                gcode_layer.addWalls(insets_that_do_not_surround_holes, inset_level, mesh, mesh_config.insetX_config, mesh_config.bridge_insetX_config, wall_overlapper_x);
    ...
    for (unsigned int outer_poly_order_idx = 0; outer_poly_order_idx < order_optimizer.polyOrder.size(); ++outer_poly_order_idx)
    {
        ...
        if (hole_inner_walls.size() > 0 && extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_x_extruder_nr").extruder_nr)
        {
            ...
            if (outer_inset_first)
            {
                ...
                {
                    gcode_layer.addWalls(hole_outer_wall, 0, mesh, mesh_config.inset0_config, mesh_config.bridge_inset0_config, wall_overlapper_0, z_seam_config, wall_0_wipe_dist, flow, retract_before_outer_wall);
                }
                gcode_layer.addWalls(hole_inner_walls, 0, mesh, mesh_config.insetX_config, mesh_config.bridge_insetX_config, wall_overlapper_x);
            }
            else
            {
                ...
                gcode_layer.addWalls(hole_inner_walls, 2, mesh, mesh_config.insetX_config, mesh_config.bridge_insetX_config, wall_overlapper_x);
                if (extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_0_extruder_nr").extruder_nr)
                {
                    gcode_layer.addWall(hole_outer_wall[0], 0, outer_poly_start_idx, mesh, mesh_config.inset0_config, mesh_config.bridge_inset0_config, wall_overlapper_0, wall_0_wipe_dist, flow, retract_before_outer_wall);
        ...
        else if (extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_0_extruder_nr").extruder_nr)
        {
            ...
            if (z_seam_config.type == EZSeamType::SHARPEST_CORNER && inset_polys[0].size() == 2 && PolygonUtils::polygonOutlinesAdjacent(*inset_polys[0][1], *inset_polys[0][0], max_gap * 2))
            {
                ...
                gcode_layer.addWall(hole_outer_wall.back(), 0, point_idx, mesh, mesh_config.inset0_config, mesh_config.bridge_inset0_config, wall_overlapper_0, wall_0_wipe_dist, flow, retract_before_outer_wall);
            }
            else
            {
                gcode_layer.addWalls(hole_outer_wall, 0, mesh, mesh_config.inset0_config, mesh_config.bridge_inset0_config, wall_overlapper_0, z_seam_config, wall_0_wipe_dist, flow, retract_before_outer_wall);
...
void InsetOrderOptimizer::processOuterWallInsets(const bool include_outer, const bool include_inners)
{
    ...
    if (part_inner_walls.size() > 0 && extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_x_extruder_nr").extruder_nr)
    {
        ...
        std::function<void(void)> addInnerWalls = [this, part_inner_walls, outer_inset_first, z_seam_location]()
        {
            ...
            {
                gcode_layer.addWall(part_inner_walls[wall_idx], 2, orderOptimizer.polyStart[wall_idx], mesh, mesh_config.insetX_config, mesh_config.bridge_insetX_config, wall_overlapper_x, wall_0_wipe_dist, flow_ratio, always_retract);
            }
        };
        ...
        if (outer_inset_first)
        {
            ...
            {
                gcode_layer.addWall(*inset_polys[0][0], 0, outer_poly_start_idx, mesh, mesh_config.inset0_config, mesh_config.bridge_inset0_config, wall_overlapper_0, wall_0_wipe_dist, flow, retract_before_outer_wall);
            }
            addInnerWalls();
        }
        else
    ...
    else if (include_outer && extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_0_extruder_nr").extruder_nr)
    {
        ...
        gcode_layer.addWalls(part_outer_wall, 0, mesh, mesh_config.inset0_config, mesh_config.bridge_inset0_config, wall_overlapper_0, z_seam_config, wall_0_wipe_dist, flow, retract_before_outer_wall);
        // move inside so an immediately following retract doesn't occur on the outer wall
        moveInside();
        added_something = true;
    }
...
bool InsetOrderOptimizer::processInsetsWithOptimizedOrdering()
{
    ...
    if (extruder_nr == mesh.settings.get<ExtruderTrain&>("wall_x_extruder_nr").extruder_nr)
    {
        ...
        if (remaining.size() > 0)
        {
            gcode_writer.setExtruder_addPrime(storage, gcode_layer, extruder_nr);
            gcode_layer.setIsInside(true); // going to print stuff inside print object
            gcode_layer.addWalls(remaining, 2, mesh, mesh_config.insetX_config, mesh_config.bridge_insetX_config, wall_overlapper_x);
            added_something = true;
        }
    }
    if (wall_overlapper_0)
