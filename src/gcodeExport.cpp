...
//#ifndef ADD_GCODE_COMMENTS
//#define ADD_GCODE_COMMENTS
//#endif

//#ifndef PICASO_DBG_E_DETAILS
//#define PICASO_DBG_E_DETAILS
//#endif
...
, current_width_factor(Ratio(1.0))
, current_speed_factor(Ratio(1.0))
, layer_nr(0)
...
, relative_extrusion(false)
{
    *output_stream << std::fixed;
    ...
    picasoPrintModeSpeed[(int)PicasoPrintMode::HighQuality][(int)PicasoSpeedProfile::Infill] = 20.0;
...
void GCodeExport::preSetup(const size_t start_extruder)
{
    ...
    firmware_retract = mesh_group->settings.get<bool>("machine_firmware_retract"); // PicasoAddon
    firmware_zhopp = mesh_group->settings.get<bool>("machine_firmware_zhopp"); // PicasoAddon
    unretract_after_change = mesh_group->settings.get<bool>("machine_unretract_after_change"); // PicasoAddon
    enable_g1_wu_axis = mesh_group->settings.get<bool>("enable_g1_wu_axis"); // PicasoAddon
    ...
    hop_when_extruder_switch = mesh_group->settings.get<bool>("retraction_hop_when_extruder_switch"); // PicasoAddon
    ...
    total_print_times_pathConfig.ReInit(mesh_group->settings.get<size_t>("machine_extruder_count")); // PicasoAddon
...
std::string GCodeExport::getFileHeader(const std::vector<bool>& extruder_is_used, const Duration* print_time, const std::vector<double>& filament_used, const std::vector<std::string>& mat_ids)
{
    ...
    case EGCodeFlavor::PICASO:
    {
        ...
        // <print_task_stats !!!!!!!!!>
        // 2DO: remake protocol for new discrete version per tool
        print_task_stats.zhopp_count = 0;
        print_task_stats.retract_count = 0;
        for (size_t n = 0; n < total_estimates.extruder_count; n++)
        {
            print_task_stats.zhopp_count += total_estimates.zhopp_count[n];
            print_task_stats.retract_count += total_estimates.retract_count[n];
        }
        // </print_task_stats>
        ...
        if (scene.settings.get<bool>("cmd_enable_m1114")) // Report Layer Additional Counters
        {
            ...
            {
                prefix << ";M1114"
                       << " T" << n << " Z" << total_estimates.zhopp_count[n] << " E" << total_estimates.retract_count[n] << " P"
                       << PrecisionedDouble{ 3, total_estimates.extra_time } << new_line;
            }
        }
        ...
        {
            for (auto it = total_print_times_pathConfig.durations.begin(); it != total_print_times_pathConfig.durations.end();
                 ++it) // Nozzle
            {
        ...
        // prefix << ";POLYGON_BASIC_FEEDRATE=" <<
        // Application::getInstance().current_slice->scene.current_mesh_group->settings.get<double>("speed_print") << new_line;
...
std::string GCodeExport::getPathConfigFeatureConfig(const Scene& scene,
...
std::string GCodeExport::getFileDurationEstimateReport(const size_t extruder, const PathConfigFeature& feature, const Duration& duration)
{
    std::ostringstream prefix;
    prefix << ";M1115 T" << extruder;
    prefix << " S" << static_cast<int>(feature);
    prefix << " P" << PrecisionedDouble{ 3, duration } << new_line;
    return prefix.str();
}
...
int GCodeExport::getExtrudersUsed() const
{
    ...
    const EGCodeFlavor flavor = Application::getInstance().current_slice->scene.settings.get<EGCodeFlavor>("machine_gcode_flavor"); // PicasoAddon
    ...
    for (const MeshGroup& mesh_group : Application::getInstance().current_slice->scene.mesh_groups)
    {
        ...
        {
            const Settings& mesh_settings = mesh.settings; // Picaso Patch
            if (mesh_settings.get<bool>("support_enable") || mesh_settings.get<bool>("support_mesh"))
            {
                ret[mesh_settings.get<ExtruderTrain&>("support_extruder_nr_layer_0").extruder_nr] = true;
                ret[mesh_settings.get<ExtruderTrain&>("support_infill_extruder_nr").extruder_nr] = true;
                if (mesh_settings.get<bool>("support_roof_enable"))
                {
                    ret[mesh_settings.get<ExtruderTrain&>("support_roof_extruder_nr").extruder_nr] = true;
                }
                if (flavor == EGCodeFlavor::PICASO)
                {
                    if (mesh_settings.get<bool>("support_under_roof_enable"))
                    {
                        ret[mesh_settings.get<ExtruderTrain&>("support_under_roof_extruder_nr").extruder_nr] = true;
                    }
                    if (mesh_settings.get<bool>("support_above_bottom_enable"))
                    {
                        ret[mesh_settings.get<ExtruderTrain&>("support_above_bottom_extruder_nr").extruder_nr] = true;
                    }
                }
                if (mesh_settings.get<bool>("support_bottom_enable"))
                {
                    ret[mesh_settings.get<ExtruderTrain&>("support_bottom_extruder_nr").extruder_nr] = true;
                }
            }

            ret[mesh_settings.get<ExtruderTrain&>("wall_0_extruder_nr").extruder_nr] = true;
            ret[mesh_settings.get<ExtruderTrain&>("wall_x_extruder_nr").extruder_nr] = true;
            ret[mesh_settings.get<ExtruderTrain&>("top_bottom_extruder_nr").extruder_nr] = true;
            ret[mesh_settings.get<ExtruderTrain&>("infill_extruder_nr").extruder_nr] = true;

            if (flavor == EGCodeFlavor::PICASO)
            {
                ret[mesh_settings.get<ExtruderTrain&>("wall_1_extruder_nr").extruder_nr] = true;
                ret[mesh_settings.get<ExtruderTrain&>("roofing_extruder_nr").extruder_nr] = true;
                ret[mesh_settings.get<ExtruderTrain&>("flooring_extruder_nr").extruder_nr] = true;
            }
        }
    }
...
{
    PicasoPrintModeEstimate result(
      estimateResult.extruder_count, estimateResult.extra_time, estimateResult.retract_count, estimateResult.zhopp_count);
    ...
    if (getFlavor() == EGCodeFlavor::PICASO)
    {
            ...
            for (auto it = layerEstimates.durations.begin(); it != layerEstimates.durations.end(); ++it)
            {
                for (size_t n = 0; n < it->second.size(); n++)
                {
                    *output_stream << "M1111 T" << it->first 
                        ...
                        << " S" << n 
                        << " P" << PrecisionedDouble{ 3, it->second[n] } << new_line;
                }
            }
        }
        if (scene.settings.get<bool>("cmd_enable_m1113")) // Report Layer Additional Counters
        {
            for (size_t n = 0; n < layerEstimates.extruder_count; n++)
            {
                *output_stream << "M1113 T" << n 
                    << " L" << this->zero_based_layer_nr 
                    << " Z" << layerEstimates.zhopp_count[n] 
                    << " E" << layerEstimates.retract_count[n] 
                    << " P" << PrecisionedDouble{ 3, layerEstimates.extra_time } << new_line;
            }
        }
                ...
                for (size_t n = 0; n < it->second.size(); n++) // PathConfigFeature
                {
                    ...
                    { // PLX-702
                        *output_stream << ";M1116 L" << this->zero_based_layer_nr 
                            ...
                            << " P" << PrecisionedDouble{ 3, duration } << new_line;
                    }
                }
            }
        }
        ...
        if (scene.settings.get<bool>("cmd_enable_m1114"))
        {
            for (size_t n = 0; n < layerEstimates.extruder_count; n++)
            {
                ...
                *output_stream << "M1114 T" << n 
                    << " Z" << totalEstimates.zhopp_count[n] 
                    << " E" << totalEstimates.retract_count[n] 
                    << " P" << PrecisionedDouble{ 3, totalEstimates.extra_time } << new_line;
            }
        }
        // </Total>
    }
...
Velocity GCodeExport::getFeatureSpeed(const Scene& scene, const PathConfigFeature& feature)
{
    switch (feature)
    {
    case PathConfigFeature::Inset0:
        return scene.settings.get<Velocity>("speed_wall_0");
    case PathConfigFeature::Inset1:
        return scene.settings.get<Velocity>("speed_wall_1");
    case PathConfigFeature::InsetX:
        return scene.settings.get<Velocity>("speed_wall_x");
    case PathConfigFeature::BridgeInset0:
        return scene.settings.get<Velocity>("bridge_wall_speed");
    case PathConfigFeature::BridgeInset1:
        return scene.settings.get<Velocity>("bridge_wall_speed");
    case PathConfigFeature::BridgeInsetX:
        return scene.settings.get<Velocity>("bridge_wall_speed");
    case PathConfigFeature::BridgeSkin1:
        return scene.settings.get<Velocity>("bridge_skin_speed");
    case PathConfigFeature::BridgeSkin2:
        return scene.settings.get<Velocity>("bridge_skin_speed_2");
    case PathConfigFeature::BridgeSkin3:
        return scene.settings.get<Velocity>("bridge_skin_speed_3");
    case PathConfigFeature::Skin:
        return scene.settings.get<Velocity>("speed_topbottom");
    case PathConfigFeature::Roofing:
        return scene.settings.get<Velocity>("speed_roofing");
    case PathConfigFeature::Flooring:
        return scene.settings.get<Velocity>("speed_flooring");
    case PathConfigFeature::Infill:
        return scene.settings.get<Velocity>("speed_infill");
    case PathConfigFeature::Ironing:
        return scene.settings.get<Velocity>("speed_ironing");
    case PathConfigFeature::PerimeterGap:
        return scene.settings.get<Velocity>("speed_topbottom");
    case PathConfigFeature::RaftBase:
        return scene.settings.get<Velocity>("raft_base_speed");
    case PathConfigFeature::RaftInterface:
        return scene.settings.get<Velocity>("raft_interface_speed");
    case PathConfigFeature::RaftSurface:
        return scene.settings.get<Velocity>("raft_surface_speed");
    case PathConfigFeature::ExtruderTravel:
        return scene.settings.get<Velocity>("speed_travel");
    case PathConfigFeature::ExtruderSkirtBrim:
        return scene.settings.get<Velocity>("skirt_brim_speed");
    case PathConfigFeature::ExtruderPrimeTower:
        return scene.settings.get<Velocity>("speed_prime_tower");
    case PathConfigFeature::SupportRoof:
        return scene.settings.get<Velocity>("speed_support_roof");
    case PathConfigFeature::SupportInfill:
        return scene.settings.get<Velocity>("speed_support_infill");
    case PathConfigFeature::SupportBottom:
        return scene.settings.get<Velocity>("speed_support_bottom");
    case PathConfigFeature::SupportUnderRoof:
        return scene.settings.get<Velocity>("speed_support_under_roof");
    case PathConfigFeature::SupportAboveBottom:
        return scene.settings.get<Velocity>("speed_support_above_bottom");
    default:
        return Velocity(1.0);
    }
}
...
void GCodeExport::writeLayerComment(const LayerIndex layer_nr, const coord_t z, const coord_t layer_thickness)
{
    if (this->flavor == EGCodeFlavor::PICASO)
    {
        *output_stream << ";LAYER:" << this->zero_based_layer_nr << new_line;
        *output_stream << "M532 L" << this->zero_based_layer_nr << " Z" << MMtoStream{ z } << " R" << MMtoStream{ layer_thickness }
                       << new_line;
    }
    else
    {
...
void GCodeExport::writeExtrusion(const Point& p,
                                 const Velocity& speed,
								 double extrusion_mm3_per_mm,
								 const PrintFeatureType& feature,
								 const PathConfigFeature& featureType, // PicasoAddon
								 const Ratio& width_factor, // PicasoAddon
								 const Ratio& speed_factor, // PicasoAddon
								 bool update_extrusion_offset)
{
    writeExtrusion(Point3(p.X, p.Y, current_layer_z), speed, extrusion_mm3_per_mm, feature, featureType, width_factor, speed_factor, update_extrusion_offset);
}
...
void GCodeExport::writeExtrusion(const Point3& p,
                                 const Velocity& speed,
								 double extrusion_mm3_per_mm,
								 const PrintFeatureType& feature,
								 const PathConfigFeature& featureType, // PicasoAddon
								 ...
								 bool update_extrusion_offset)
{
    ...
    writeExtrusion(p.x, p.y, p.z, speed, extrusion_mm3_per_mm, feature, featureType, width_factor, speed_factor, update_extrusion_offset);
}

void GCodeExport::writeMoveBFB(const int x,
                               const int y,
							   const int z,
							   const Velocity& speed,
							   double extrusion_mm3_per_mm,
...
void GCodeExport::writeTravel(const coord_t x, const coord_t y, const coord_t z, const Velocity& speed)
{
    ...
    const PathConfigFeature travel_feature_type = PathConfigFeature::ExtruderTravel; // PicasoAddon
...
void GCodeExport::writeExtrusion(const coord_t x,
                                 const coord_t y,
                                 const coord_t z,
                                 const Velocity& speed,
                                 ...
                                 const bool update_extrusion_offset)
{
    ...
    first_movement_on_layer = false; // PicasoAddon
...
#ifdef PICASO_DBG_E_DETAILS
    // <PicasoAddon>
    *output_stream << ";DBG writeExtrusion curE: " << current_e_value << ", newE:" << new_e_value 
        << ", lastE:" << extruder_attr[current_extruder].last_e_value_after_wipe 
        << ", rEstart:" << extruder_attr[current_extruder].retraction_e_amount_at_e_start 
        << ", rEcurr:" << extruder_attr[current_extruder].retraction_e_amount_current
        << new_line;
    // </PicasoAddon>
#endif // PICASO_DBG_E_DETAILS

    *output_stream << "G1";
    if (flavor == EGCodeFlavor::PICASO && enable_g1_wu_axis)
    {
        writeFXYZEWU(speed, x, y, z, new_e_value, feature, featureType, width_factor, speed_factor);
    }
    else
    {
...
void GCodeExport::writeFXYZE(const Velocity& speed,
                             const coord_t x,
							 const coord_t y,
							 const coord_t z,
							 const double e,
...
// <PicasoAddon>
    ...
    *output_stream << " ;T" << current_extruder << "; TYPE:" << printFeatureTypeToString(feature) << "; F:" << PrecisionedDouble{ 1, speed * 60 };
...
void GCodeExport::writeFXYZEWU(const Velocity& speed,
                               const coord_t x,
							   const coord_t y,
							   const coord_t z,
							   const double e,
							   const PrintFeatureType& feature,
							   const PathConfigFeature& featureType,
							   ...
							   const Ratio& speed_factor)
{
    ...
    *output_stream << " X" << MMtoStream{ gcode_pos.X } << " Y" << MMtoStream{ gcode_pos.Y };
    if (z != currentPosition.z) // Z axis
    {
        *output_stream << " Z" << MMtoStream{ z };
    }
    if (e + current_e_offset != current_e_value) // E axis
    {
        const double output_e = (relative_extrusion) ? e + current_e_offset - current_e_value : e + current_e_offset;
        *output_stream << " " << extruder_attr[current_extruder].extruderCharacter << PrecisionedDouble{ 5, output_e };
    }

    constexpr double delta_epsilon = 0.00001;
    if (std::abs(current_width_factor - width_factor) > delta_epsilon) // W axis (width factor)
    {
        *output_stream << " W" << PrecisionedDouble{ 5, width_factor };
    }
    if (std::abs(current_speed_factor - speed_factor) > delta_epsilon) // U axis (speed factor)
    {
        *output_stream << " U" << PrecisionedDouble{ 5, speed_factor };
    }

#ifdef ADD_GCODE_COMMENTS
    *output_stream << " ;T" << current_extruder << "; TYPE:" << printFeatureTypeToString(feature)
                   << "; F:" << PrecisionedDouble{ 1, speed * 60 };
#endif // ADD_GCODE_COMMENTS
    ...
    *output_stream << new_line;
    ...
    currentPosition = Point3(x, y, z);
    current_e_value = e;
    current_width_factor = width_factor;
...
void GCodeExport::writeUnretractionAndPrime()
{
    ...
    if (extruder_attr[current_extruder].retraction_e_amount_current)
    {
        ...
        { // note that BFB is handled differently
            ...
            if (flavor == EGCodeFlavor::PICASO)
            {
                Velocity speed = extruder_attr[current_extruder].last_retraction_prime_speed;
                ...
                *output_stream << "G11 F" << PrecisionedDouble{ 1, speed * 60 } << " E" << PrecisionedDouble{ 5, output_e };
                ...
                estimateCalculator.addRetract(false, current_extruder);
            }
            else
#endif
            {
    ...
    if (getCurrentExtrudedVolume() > 10000.0 
        && flavor != EGCodeFlavor::BFB
        && flavor != EGCodeFlavor::MAKERBOT) 
    {
        resetExtrusionValue();
    }
    if (extruder_attr[current_extruder].retraction_e_amount_current)
...
void GCodeExport::writeRetraction(const RetractionConfig& config, bool force, bool extruder_switch)
{
    ...
    if (extruder_settings.get<bool>("machine_firmware_retract"))
    {
        ...
        if (flavor == EGCodeFlavor::PICASO)
        {
            ...
            currentSpeed = speed;
            // E-axis fillament up
            *output_stream << "G10 F" << PrecisionedDouble{ 1, speed * 60 } << " E" << PrecisionedDouble{ 5, output_e };
#ifdef ADD_GCODE_COMMENTS
            if (extruder_switch)
            {
            ...
            estimateCalculator.addRetract(true, current_extruder);
        }
        else
#endif
        {
...
#ifdef PICASO_DBG_E_DETAILS
    // <PicasoAddon>
    *output_stream << ";DBG writeRetraction curE: " << current_e_value 
        << ", currE:" << current_e_value
        << ", oldRE:" << old_retraction_e_amount
        << ", newRE:" << new_retraction_e_amount
        << ", rEcurr:" << extr_attr.retraction_e_amount_current 
        << ", cfgPrime:" << config.prime_volume
        << ", cfgDist:" << config.distance
        << new_line;
    // </PicasoAddon>
#endif // PICASO_DBG_E_DETAILS
    ...
    if (hop_height > 0 && (is_z_hopped != hop_height))
    {
        ...
        if (firmware_zhopp && flavor == EGCodeFlavor::PICASO)
        {
            ...
            *output_stream << "G10 F" << PrecisionedDouble{ 1, speed * 60 } << " Z" << MMtoStream{ current_layer_z + is_z_hopped };
            estimateCalculator.addZHopp(true, current_extruder);
        }
        else
    ...
    if (is_z_hopped)
    {
        ...
        if (firmware_zhopp && flavor == EGCodeFlavor::PICASO)
        {
            ...
            *output_stream << "G11 F" << PrecisionedDouble{ 1, speed * 60 } << " Z" << MMtoStream{ current_layer_z };
            estimateCalculator.addZHopp(false, current_extruder);
        }
        else
#endif
        {
...
void GCodeExport::startExtruder(const size_t new_extruder)
{
    ...
    {
        /*
        * Bugfix: incorrect counting fillament length before reset Extruder axis
        */
        double current_extruded_volume = getCurrentExtrudedVolume();
        extruder_attr[current_extruder].totalFilament += current_extruded_volume;

        // Already counted value
        // !!! Warn: no need resetExtrusionValue() because G92 in output
        current_e_value = 0.0;
    }
    // </SL716>
                                 ...
                                 coord_t perform_z_hop /*= 0*/)
{
...
#ifdef PICASO_DBG_E_DETAILS
        // <PicasoAddon>
        *output_stream << ";DBG switchExtruder currExtr: " << current_extruder 
            << ", newExtr:" << new_extruder
            << ", currREcurr:" << extruder_attr[current_extruder].retraction_e_amount_current 
            << ", newREcurr:" << extruder_attr[new_extruder].retraction_e_amount_current 
            << ", newREnew:" << retraction_config_new_extruder.distance 
            << new_line;
        // </PicasoAddon>
#endif // PICASO_DBG_E_DETAILS
    ...
    if (flavor == EGCodeFlavor::PICASO && hop_when_extruder_switch)
    {
...
#ifdef PICASO_DBG_E_DETAILS
    // <PicasoAddon>
    *output_stream << ";DBG resetExtruderToPrimed extruder: " << extruder 
        << ", currE:" << current_e_value
        << ", rEcurr:" << extruder_attr[extruder].retraction_e_amount_current 
        << ", rEnew:" << initial_retraction
        << new_line;
    // </PicasoAddon>
#endif // PICASO_DBG_E_DETAILS
...
void GCodeExport::finalize(const char* endCode)
{
    ...
    if (flavor == EGCodeFlavor::PICASO)
    {
        ...
        lines << "; Print time (readable): " << int(print_time / 60 / 60) << "h " << int((print_time / 60) % 60) << "m "
              << int(print_time % 60) << "s" << new_line;
