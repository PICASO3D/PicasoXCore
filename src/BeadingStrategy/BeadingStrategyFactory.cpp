...
BeadingStrategyPtr BeadingStrategyFactory::makePicasoStrategy
(
    const coord_t preferred_bead_width_outer_0,
    const coord_t preferred_bead_width_outer_1,
    const coord_t preferred_bead_width_inner,
    const coord_t preferred_transition_length,
    const float transitioning_angle,
    const bool print_thin_walls,
...
)
{
    ...
    // Inner Inset_x
    BeadingStrategyPtr ret = make_unique<DistributedBeadingStrategy>(preferred_bead_width_inner, preferred_transition_length, transitioning_angle, wall_split_middle_threshold, wall_add_middle_threshold, inward_distributed_center_wall_count);
    spdlog::debug("Applying the Redistribute meta-strategy with outer-wall width = {}, inner-wall width = {}", preferred_bead_width_outer_0, preferred_bead_width_inner);

    // Outer Inset_1
    ret = make_unique<RedistributeBeadingStrategy>(preferred_bead_width_outer_1, minimum_variable_line_ratio, move(ret));

    // Outer Inset_0
    ret = make_unique<RedistributeBeadingStrategy>(preferred_bead_width_outer_0, minimum_variable_line_ratio, move(ret));
