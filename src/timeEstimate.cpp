...
void TimeEstimateCalculator::setFirmwareDefaults(const Settings& settings)
{
    ...
    {
        retract_start.push_back(0);
        retract_end.push_back(0);
        zhopp_start.push_back(0);
        zhopp_end.push_back(0);
    }
}
...
void TimeEstimateCalculator::addRetract(bool start, size_t extruder)
{
    if (start)
        retract_start[extruder]++;
    else
        retract_end[extruder]++;
}
void TimeEstimateCalculator::addZHopp(bool start, size_t extruder)
{
    if (start)
        zhopp_start[extruder]++;
    else
        zhopp_end[extruder]++;
}

double TimeEstimateCalculator::getMaxXFeedrate()
{
    return max_feedrate[X_AXIS];
}
...
void TimeEstimateCalculator::plan(Position newPos, Velocity feedrate, PrintFeatureType feature, PathConfigFeature featureType, size_t extruder)
{
    Block block;
    memset(&block, 0, sizeof(block));

    block.feature = feature;
    block.featureType = featureType;
...
TimeEstimateResult TimeEstimateCalculator::getTimeEstimateResult()
{
    ...
    for (unsigned int n = 0; n < blocks.size(); n++)
    {
        ...
        cura::Duration value;
        value += accelerationTimeFromDistance(block.initial_feedrate, block.accelerate_until, block.acceleration);
        value += plateau_distance / block.nominal_feedrate;
        value += accelerationTimeFromDistance(block.final_feedrate, (block.distance - block.decelerate_after), block.acceleration);
