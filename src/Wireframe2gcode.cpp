...
void Wireframe2gcode::writeGCode()
{
    ...
    for (PolygonRef bottom_part : wireFrame.bottom_infill.roof_outlines)
    {
        ...
        {
            gcode.writeExtrusion(segment_to, speedBottom, extrusion_mm3_per_mm_flat, PrintFeatureType::Skin, PathConfigFeature::Skin);
        }
    }
    ...
    writeFill(wireFrame.bottom_infill.roof_insets, empty_outlines, 
              [this](Wireframe2gcode&, WeaveConnectionPart& part, unsigned int segment_idx) {
                    ...
                    {
                        gcode.writeExtrusion(segment.to, speedBottom, extrusion_mm3_per_mm_connection, PrintFeatureType::Skin, PathConfigFeature::Skin);
                    }
                }   
            , 
              [this](Wireframe2gcode&, WeaveConnectionSegment& segment) {
                    if (segment.segmentType == WeaveSegmentType::MOVE)
                        ...
                        gcode.writeExtrusion(segment.to, speedBottom, extrusion_mm3_per_mm_flat, PrintFeatureType::Skin, PathConfigFeature::Skin);
                }
            );
    ...
    {
        Progress::messageProgress(Progress::Stage::EXPORT, layer_nr+1, total_layers); // abuse the progress system of the normal mode of PicasoXCore
        ...
        for (size_t part_nr = 0; part_nr < layer.connections.size(); part_nr++)
        {
            ...
            gcode.writeTypeComment(PrintFeatureType::OuterWall); // top
            {
                for (WeaveConnectionSegment& segment : part.connection.segments)
                {
                    ...
                    {
                        gcode.writeExtrusion(segment.to, speedFlat, extrusion_mm3_per_mm_flat, PrintFeatureType::OuterWall, PathConfigFeature::Inset0);
        ...
        writeFill(layer.roofs.roof_insets, layer.roofs.roof_outlines,
                ...
                [this](Wireframe2gcode&, WeaveConnectionSegment& segment) { // handle flat segments
                    ...
                    {   
                        gcode.writeExtrusion(segment.to, speedFlat, extrusion_mm3_per_mm_flat, PrintFeatureType::Skin, PathConfigFeature::Roofing);
                        gcode.writeDelay(flat_delay);
                    }
                });
...
void Wireframe2gcode::go_down(WeaveConnectionPart& part, unsigned int segment_idx)
{
    ...
    {
        gcode.writeExtrusion(segment.to, speedDown, extrusion_mm3_per_mm_connection, PrintFeatureType::OuterWall, PathConfigFeature::Inset0);
    } else 
    {
        ...
        gcode.writeExtrusion(up, speedDown*enlargement, extrusion_mm3_per_mm_connection / enlargement, PrintFeatureType::OuterWall, PathConfigFeature::Inset0);
        gcode.writeExtrusion(to, speedDown*enlargement, extrusion_mm3_per_mm_connection / enlargement, PrintFeatureType::OuterWall, PathConfigFeature::Inset0);
    }
...
{
    WeaveConnectionSegment& segment = part.connection.segments[segment_idx];
    gcode.writeExtrusion(segment.to, speedUp, extrusion_mm3_per_mm_connection, PrintFeatureType::OuterWall, PathConfigFeature::Inset0);
    Point3 next_vector;
...
void Wireframe2gcode::strategy_retract(WeaveConnectionPart& part, unsigned int segment_idx)
{
    ...
    if (lower_retract_start)
    {
        ...
        gcode.writeExtrusion(lower, speedUp, extrusion_mm3_per_mm_connection, PrintFeatureType::OuterWall, PathConfigFeature::Inset0);
    ...
    {
        gcode.writeExtrusion(to, speedUp, extrusion_mm3_per_mm_connection, PrintFeatureType::OuterWall, PathConfigFeature::Inset0);
        gcode.writeRetraction(retraction_config);
        gcode.writeTravel(to + Point3(0, 0, retract_hop_dist), speedFlat);
        gcode.writeDelay(top_retract_pause);
        if (after_retract_hop)    
...
void Wireframe2gcode::strategy_compensate(WeaveConnectionPart& part, unsigned int segment_idx)
{
    ...
    gcode.writeExtrusion(newTop, speedUp * newLength / orrLength, extrusion_mm3_per_mm_connection * orrLength / newLength, PrintFeatureType::OuterWall, PathConfigFeature::Inset0);
}
        ...
        case WeaveSegmentType::UP:
            {
                ...
                gcode.writeExtrusion(detoured, speedUp, extrusion_mm3_per_mm_connection, PrintFeatureType::Skin, PathConfigFeature::Roofing);

            }
            break;
        case WeaveSegmentType::DOWN:
            gcode.writeExtrusion(segment.to, speedDown, extrusion_mm3_per_mm_connection, PrintFeatureType::Skin, PathConfigFeature::Roofing);
...
void Wireframe2gcode::processStartingCode()
{
    ...
    gcode.writeComment("Generated with PicasoCXore " VERSION);
...
void Wireframe2gcode::processSkirt()
{
    ...
    for (size_t poly_order_idx = 0; poly_order_idx < skirt.size(); poly_order_idx++)
    {
        ...
        for (unsigned int point_idx = 0; point_idx < poly.size(); point_idx++)
        {
            ...
            gcode.writeExtrusion(p, scene_settings.get<Velocity>("skirt_brim_speed"), scene_settings.get<double>("skirt_brim_line_width") * scene_settings.get<Ratio>("initial_layer_line_width_factor") * INT2MM(initial_layer_thickness), PrintFeatureType::SkirtBrim, PathConfigFeature::ExtruderSkirtBrim);
        }
    }
}
