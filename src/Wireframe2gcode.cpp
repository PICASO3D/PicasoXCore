...
void Wireframe2gcode::writeGCode()
{
    ...
    for (PolygonRef bottom_part : wireFrame.bottom_infill.roof_outlines)
    {
        ...
        {
            gcode.writeExtrusion(segment_to, speedBottom, extrusion_mm3_per_mm_flat, PrintFeatureType::Skin, PathConfigFeature::Skin, Ratio(), Ratio());
        }
    }
    ...
    writeFill(
        ...
        [this](Wireframe2gcode&, WeaveConnectionPart& part, unsigned int segment_idx)
        {
            ...
            {
                gcode.writeExtrusion(segment.to, speedBottom, extrusion_mm3_per_mm_connection, PrintFeatureType::Skin, PathConfigFeature::Skin, Ratio(), Ratio());
            }
        },
        [this](Wireframe2gcode&, WeaveConnectionSegment& segment)
        {
            if (segment.segmentType == WeaveSegmentType::MOVE)
                ...
                gcode.writeExtrusion(segment.to, speedBottom, extrusion_mm3_per_mm_flat, PrintFeatureType::Skin, PathConfigFeature::Skin, Ratio(), Ratio());
        });
    ...
    for (LayerIndex layer_nr = 0; layer_nr < static_cast<LayerIndex>(wireFrame.layers.size()); layer_nr++)
    {
        ...
        for (size_t part_nr = 0; part_nr < layer.connections.size(); part_nr++)
        {
            ...
            gcode.writeTypeComment(PrintFeatureType::OuterWall); // top
            {
                for (WeaveConnectionSegment& segment : part.connection.segments)
                {
                    ...
                    {
                        gcode.writeExtrusion(segment.to, speedFlat, extrusion_mm3_per_mm_flat, PrintFeatureType::OuterWall, PathConfigFeature::Inset0, Ratio(), Ratio());
        ...
        writeFill(layer.roofs.roof_insets, layer.roofs.roof_outlines, handle_roof, [this](Wireframe2gcode&, WeaveConnectionSegment& segment) { // handle flat segments
            ...
            {
                gcode.writeExtrusion(segment.to, speedFlat, extrusion_mm3_per_mm_flat, PrintFeatureType::Skin, PathConfigFeature::Roofing, Ratio(), Ratio());
                gcode.writeDelay(flat_delay);
            }
        });
    }
...
void Wireframe2gcode::go_down(WeaveConnectionPart& part, unsigned int segment_idx)
{
    ...
    {
        gcode.writeExtrusion(segment.to, speedDown, extrusion_mm3_per_mm_connection, PrintFeatureType::OuterWall, PathConfigFeature::Inset0, Ratio(), Ratio());
    }
    else
    {
        ...
        gcode.writeExtrusion(up, speedDown*enlargement, extrusion_mm3_per_mm_connection / enlargement, PrintFeatureType::OuterWall, PathConfigFeature::Inset0, Ratio(), Ratio());
        gcode.writeExtrusion(to, speedDown*enlargement, extrusion_mm3_per_mm_connection / enlargement, PrintFeatureType::OuterWall, PathConfigFeature::Inset0, Ratio(), Ratio());
    }
    gcode.writeDelay(bottom_delay);
    if (up_dist_half_speed > 0)
    {
        gcode.writeExtrusion(Point3(0, 0, up_dist_half_speed) + gcode.getPosition(), speedUp / 2, extrusion_mm3_per_mm_connection * 2, PrintFeatureType::OuterWall, PathConfigFeature::Inset0, Ratio(), Ratio());
    }
}
...
{
    WeaveConnectionSegment& segment = part.connection.segments[segment_idx];
    gcode.writeExtrusion(segment.to, speedUp, extrusion_mm3_per_mm_connection, PrintFeatureType::OuterWall, PathConfigFeature::Inset0, Ratio(), Ratio());
    Point3 next_vector;
...
void Wireframe2gcode::strategy_retract(WeaveConnectionPart& part, unsigned int segment_idx)
{
    ...
    if (lower_retract_start)
    {
        ...
        gcode.writeExtrusion(lower, speedUp, extrusion_mm3_per_mm_connection, PrintFeatureType::OuterWall, PathConfigFeature::Inset0, Ratio(), Ratio());
    ...
    {
        gcode.writeExtrusion(to, speedUp, extrusion_mm3_per_mm_connection, PrintFeatureType::OuterWall, PathConfigFeature::Inset0, Ratio(), Ratio());
        gcode.writeRetraction(retraction_config);
        gcode.writeTravel(to + Point3(0, 0, retract_hop_dist), speedFlat);
        gcode.writeDelay(top_retract_pause);
        if (after_retract_hop)
...
void Wireframe2gcode::strategy_compensate(WeaveConnectionPart& part, unsigned int segment_idx)
{
    ...
    gcode.writeExtrusion(newTop, speedUp * newLength / orrLength, extrusion_mm3_per_mm_connection * orrLength / newLength, PrintFeatureType::OuterWall, PathConfigFeature::Inset0, Ratio(), Ratio());
}
...
void Wireframe2gcode::handle_roof_segment(WeaveConnectionPart& part, unsigned int segment_idx)
{
    ...
    case WeaveSegmentType::UP:
    {
        ...
        gcode.writeExtrusion(detoured, speedUp, extrusion_mm3_per_mm_connection, PrintFeatureType::Skin, PathConfigFeature::Roofing, Ratio(), Ratio());
    }
    break;
    case WeaveSegmentType::DOWN:
        gcode.writeExtrusion(segment.to, speedDown, extrusion_mm3_per_mm_connection, PrintFeatureType::Skin, PathConfigFeature::Roofing, Ratio(), Ratio());
...
void Wireframe2gcode::processStartingCode()
{
    ...
    gcode.writeComment("Generated with PicasoCXore ");
