 ...
 *
 * This converts data from PicasoXCore's internal data structures to Protobuf
...
bool ArcusCommunication::hasSlice() const
{
    return private_data->socket->getState() != Arcus::SocketState::Closed
        ...
        && private_data->slice_count < 1; //Only slice once per run of PicasoXCore. See documentation of slice_count.
}
