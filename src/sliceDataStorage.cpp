...
bool SliceMeshStorage::getExtruderIsUsed(const size_t extruder_nr) const
{
    ...
    if ((settings.get<size_t>("wall_line_count") > 1 || settings.get<bool>("alternate_extra_perimeter") || settings.get<FillPerimeterGapMode>("fill_perimeter_gaps") == FillPerimeterGapMode::EVERYWHERE)
    ...
    else 
    {
        ...
        if (include_support)
        {
            ...
            if (support.generated) 
            {
                ...
                total.add(support_layer.support_above_bottom);
                total.add(support_layer.support_under_roof);
...
std::vector<bool> SliceDataStorage::getExtrudersUsed() const
{
    ...
    for (const SliceMeshStorage& mesh : meshes)
    {
        if (mesh.settings.get<bool>("support_enable") || mesh.settings.get<bool>("support_mesh"))
        {
            ...
            if (mesh_group_settings.get<bool>("support_under_roof_enable"))
            {
                ret[mesh_group_settings.get<ExtruderTrain&>("support_under_roof_extruder_nr").extruder_nr] = true;
            }
            if (mesh_group_settings.get<bool>("support_above_bottom_enable"))
            {
                ret[mesh_group_settings.get<ExtruderTrain&>("support_above_bottom_extruder_nr").extruder_nr] = true;
            }
...
std::vector<bool> SliceDataStorage::getExtrudersUsed(LayerIndex layer_nr) const
{
    ...
    if (include_helper_parts)
    {
        ...
        if (layer_nr < int(support.supportLayers.size()))
        {
            ...
            if (!support_layer.support_above_bottom.empty())
            {
                ret[mesh_group_settings.get<ExtruderTrain&>("support_above_bottom_extruder_nr").extruder_nr] = true;
            }
