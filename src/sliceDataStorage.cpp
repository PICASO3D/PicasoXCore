...
SliceMeshStorage::SliceMeshStorage(Mesh* mesh, const size_t slice_layer_count)
    ...
    , flavor(settings.get<EGCodeFlavor>("machine_gcode_flavor"))
...
bool SliceMeshStorage::getExtruderIsUsed(const size_t extruder_nr) const
{
    ...
    const auto wall_line_count = settings.get<size_t>("wall_line_count");

    if (flavor == EGCodeFlavor::PICASO)
    {
        if (wall_line_count > 0 && settings.get<ExtruderTrain&>("wall_0_extruder_nr").extruder_nr == extruder_nr)
        {
            return true;
        }
        if (wall_line_count > 1 && settings.get<ExtruderTrain&>("wall_1_extruder_nr").extruder_nr == extruder_nr)
        {
            return true;
        }
        if ((wall_line_count > 2 || settings.get<bool>("alternate_extra_perimeter")) && settings.get<ExtruderTrain&>("wall_x_extruder_nr").extruder_nr == extruder_nr)
        {
            return true;
        }
    }
    else
    {
        ...
        if ((wall_line_count > 1 || settings.get<bool>("alternate_extra_perimeter")) && settings.get<ExtruderTrain&>("wall_x_extruder_nr").extruder_nr == extruder_nr)
        {
            return true;
        }
    }
    ...
    if (flavor == EGCodeFlavor::PICASO)
    {
        if (settings.get<size_t>("top_layers") > 0 || settings.get<size_t>("bottom_layers") > 0)
        {
            ...
            if (flooring_layer_count > 0 && settings.get<ExtruderTrain&>("flooring_extruder_nr").extruder_nr == extruder_nr)
            {
                return true;
            }
        }
...
bool SliceMeshStorage::getExtruderIsUsed(const size_t extruder_nr, const LayerIndex& layer_nr) const
{
    ...
    if (settings.get<ExtruderTrain&>("wall_0_extruder_nr").extruder_nr == extruder_nr && (wall_line_count > 0 || settings.get<size_t>("skin_outline_count") > 0))
    {
    ...
    if (flavor == EGCodeFlavor::PICASO)
    {
        ...
        // Floor skin
        if ((settings.get<size_t>("flooring_layer_count") > 0) && settings.get<ExtruderTrain&>("flooring_extruder_nr").extruder_nr == extruder_nr)
        {
            for (const SliceLayerPart& part : layer.parts)
            {
                for (const SkinPart& skin_part : part.skin_parts)
    ...
    else
    {
        ...
        if (layer_nr >= 0)
        {
            for (const SliceMeshStorage& mesh : meshes)
            {
                ...
                {
                    total.add(layer.getOutlines(external_polys_only)); // PicasoBugfix
                }
        ...
        if (include_support)
        {
            ...
            if (support.generated)
            {
                ...
                total.add(support_layer.support_above_bottom); // PicasoAddon
                total.add(support_layer.support_under_roof); // PicasoAddon
...
std::vector<bool> SliceDataStorage::getExtrudersUsed() const
{
    ...
    {
        // <PicasoBugfix>
        const Settings& mesh_settings = mesh.settings;
