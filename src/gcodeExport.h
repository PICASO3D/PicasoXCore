...
private:
    ...
    bool firmware_zhopp; //!< whether z-hopp are done in the firmware, or hardcoded in Z values.
    bool unretract_after_change; //!< unretract after change tool
...
public:
    ...
    void setLastModelOnSupportState(const bool state);

    bool getLastModelOnSupportState() const;
    ...
    void writeTimeComment(std::string feature, const double time);
     ...
     */
    void writeExtrusion(const Point& p, const Velocity& speed, double extrusion_mm3_per_mm, PrintFeatureType feature, PathConfigFeature featureType, bool update_extrusion_offset = false);
     ...
     * \param update_extrusion_offset whether to update the extrusion offset to match the current flow rate
     */
    void writeExtrusion(const Point3& p, const Velocity& speed, double extrusion_mm3_per_mm, PrintFeatureType feature, PathConfigFeature featureType, bool update_extrusion_offset = false);
private:
    /*!
     * Coordinates are build plate coordinates, which might be offsetted when extruder offsets are encoded in the gcode.
     * 
     ...
     */
    void writeMoveBFB(const int x, const int y, const int z, const Velocity& speed, double extrusion_mm3_per_mm, PrintFeatureType feature, PathConfigFeature featureType);
public:
    /*!
    ...
    /*!
     * Set used extruder for current slicing
     */
    void setExtruderUsed(const size_t extruder_id);
