...
struct PathOrderPath
{
     ...
     */
    PathOrderPath(const PathType& vertices,
                  const bool is_closed = false,
                  const size_t start_vertex = 0,
                  const bool backwards = false,
                  const int seam_idx = -1,
                  const bool unlink_last_point = false)
        : vertices(vertices)
        , start_vertex(start_vertex)
        , is_closed(is_closed)
        , backwards(backwards)
        , seam_idx(seam_idx)
        , unlink_last_point(unlink_last_point)
    {}
