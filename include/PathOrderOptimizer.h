...
public:
     ...
     */
    void addPolygon(const PathType& polygon, const int seam_idx = -1)
    {
        constexpr bool is_closed = true;
        //paths.emplace_back(polygon, is_closed);

        // <PicasoAddon>
        //logWarning("PathOrderOptimizer.addPolygon: %i\n", seam_idx);
        constexpr size_t start_vertex = 0;
        constexpr bool backwards = false;
        paths.emplace_back(polygon, is_closed, start_vertex, backwards, seam_idx);
        // </PicasoAddon>
    }
    ...
    void optimize()
    {
        ...
        if(precompute_start)
        {
            for(PathOrderPath<PathType>& path : paths)
            {
                ...
                // <PicasoAddon>
                if ((seam_config.type == EZSeamType::USER_SPECIFIED || seam_config.type == EZSeamType::SHARPEST_CORNER)
                    && path.seam_idx > -1)
                {
                    // logWarning("PathOrderOptimizer.optimize: %i\n", path.seam_idx);
                    path.start_vertex = path.seam_idx; //% path.converted->size();
                    continue;
                }
                // </PicasoAddon>
...
public:
    ...
    size_t findStartLocation(const PathOrderPath<PathType>& path, const Point& target_pos)
    {
        ...
        for(size_t i = start_from_pos; i < end_before_pos; ++i)
        {
            ...
            constexpr float score_epsilon = 25.0; // PicasoBugFix
            if (seam_config.type != EZSeamType::SKIRT_BRIM && fabs(best_score - score) <= score_epsilon)
            {
