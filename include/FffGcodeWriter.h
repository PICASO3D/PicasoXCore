...
private:
    ...
    /*!
     * The gcode file to write to when using PicasoXCore as command line tool.
     */
...
public:
     ...
     * 
     * Used when PicasoXCore is used as command line tool.
     * 
     ...
     * 
     * Used when PicasoXCore is NOT used as command line tool.
     * 
...
private:
    ...
    */
     ...
     * 
     * Write a stub header if PicasoXCore is in command line tool mode. (Cause writing the header afterwards would entail moving all gcode down.)
     * 
     * \param[in] storage where the slice data is stored.
     * \param[in] start_extruder_nr The extruder with which to start the print.
     */
    ...
    void processFlooring(const SliceDataStorage& storage, LayerPlan& gcode_layer, const SliceMeshStorage& mesh, const size_t extruder_nr, const PathConfigStorage::MeshPathConfigs& mesh_config, const SkinPart& skin_part, bool& added_something) const;
    ...
    bool addSupportUnderRoofsToGCode(const SliceDataStorage& storage, LayerPlan& gcodeLayer) const; // PicasoAddon
    ...
    bool addSupportAboveBottomsToGCode(const SliceDataStorage& storage, LayerPlan& gcodeLayer) const; // PicasoAddon

public:

    void setExtruder_picaso(const SliceDataStorage& storage, LayerPlan& gcode_layer, const size_t extruder_nr) const; // PicasoAddon
