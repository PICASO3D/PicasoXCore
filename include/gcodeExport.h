...
private:
    ...
    std::vector<std::string> custom_comment;
    ...
    double picasoPrintModeSpeed[(int)PicasoPrintMode::NumPicasoPrintModes]
                               [(int)PicasoSpeedProfile::NumPicasoSpeedProfiles]; //!< Picaso SpeedMode config
    ...
    EGCodeFlavor flavor;
    Ratio current_width_factor;
    Ratio current_speed_factor;

    std::vector<Duration> total_print_times; //!< The total estimated print time in seconds for each feature
    TimeEstimateResult total_print_times_pathConfig; // PicasoAddon
    TimeEstimateCalculator estimateCalculator;

    unsigned int layer_nr; //!< for sending travel data
    unsigned int zero_based_layer_nr; // PicasoAddon

    bool is_volumetric;
    bool firmware_retract; // PicasoAddon
    bool firmware_zhopp; // PicasoAddon
    bool unretract_after_change; // PicasoAddon
    bool enable_g1_wu_axis; // PicasoAddon
    bool relative_extrusion; //!< whether to use relative extrusion distances rather than absolute
    bool hop_when_extruder_switch; // PicasoAddon
    ...
    bool last_overhang_state; // PicasoAddon
    bool last_model_on_support_state; // PicasoAddon
...
public:
    ...
    std::string getPathConfigFeatureConfig(
        const Scene& scene, 
        const PathConfigFeature& feature, 
        const std::string& key_speed, 
        const std::string& key_acceleration, 
        const std::string& key_jerk); // PicasoAddon

    std::string getFileDurationEstimateReport(
        const size_t extruder, 
        const PathConfigFeature& feature, 
        const Duration& duration); // PicasoAddon

    void setLayerNr(unsigned int layer_nr);

    void incZeroBasedLayerNr(); // PicasoAddon
    void setLastOverhangState(const bool state); // PicasoAddon
    bool getLastOverhangState() const; // PicasoAddon
    void setLastModelOnSupportState(const bool state); // PicasoAddon
    bool getLastModelOnSupportState() const; // PicasoAddon
    ...
    void setFirstMovementOnLayer(); // PicasoAddon
    ...
    int getExtrudersUsed() const; // PicasoAddon
    ...
    PicasoPrintModeEstimate getPicasoPrintModeEstimate(const Scene& scene, const TimeEstimateResult& estimateResult); // PicasoAddon
	...
	Velocity getFeatureSpeed(const Scene& scene, const PathConfigFeature& feature); // PicasoAddon
    std::string picasoSpeedProfileToKissComment(const PicasoSpeedProfile& type); // PicasoAddon
    std::string picasoSpeedProfileToString(const PicasoSpeedProfile& type); // PicasoAddon
    std::string printFeatureTypeToString(const PrintFeatureType& type); // PicasoAddon
    ...
    void writeTimeComment(std::string feature, const double time); // PicasoAddon
    ...
    void writeLayerComment(const LayerIndex layer_nr, const coord_t z, const coord_t layer_thickness); // PicasoAddon
     ...
     */
    void writeExtrusion(const Point& p,
     ...
     * \param update_extrusion_offset whether to update the extrusion offset to match the current flow rate
     */
    void writeExtrusion(const Point3& p,
                        const Velocity& speed,
                        double extrusion_mm3_per_mm,
                        const PrintFeatureType& feature,
                        const PathConfigFeature& featureType,
  ...
  private:
     ...
     * \param update_extrusion_offset whether to update the extrusion offset to match the current flow rate
     */
    void writeExtrusion(const coord_t x,
                        const coord_t y,
                        const coord_t z,
                        const Velocity& speed,
     ...
     */
    void writeFXYZE(const Velocity& speed,
                    const coord_t x,
                    const coord_t y,
                    const coord_t z,
                    const double e,
    ...
    void writeFXYZEWU(const Velocity& speed,
                    const coord_t x,
                    const coord_t y,
                    const coord_t z,
                    const double e,
  ...
  public:
    ...
    void setExtruderUsed(const size_t extruder_id); // PicasoAddon
