...
enum class EModelSkinMode
{
    NORMAL, // Top-Inner-Infill-Inner-Bottom skin mode
    EXPANDED_TOP, // Normal with Top/Bottom expansions, Top priority
    EXPANDED_BOTTOM, // Normal with Top/Bottom expansions, Bottom priority
    INNER_ONLY // Inner-Infill-Inner skin mode
};
...
enum class MagicPrintOrder
{
    SUPPORT,
    MODEL
};

enum class PicasoInitScenario
{
    DEFAULT,
    SERIES2
};
...
/**
* Picaso based GCode
*  Uses less settings on the slicer and puts more settings in the firmware. This makes for more hardware/material independed GCode.
*  G0 for moves, G1 for extrusion.
*  T0 mean select tool, T10 mean select tool and wipe clean it
**/
    PICASO = 9,
};
...
enum class InsetDirection
{
    ...
    /*
    * The second wall is printed first, then the first wall, then line in OUTSIDE_IN.
    */
    OREDER_10_IN
};
