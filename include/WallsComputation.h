...
class WallsPicasoOverride
{

  public:
    WallsPicasoOverride(const Settings& settings, const LayerIndex layer_nr);

    const std::vector<VariableWidthLines>& getToolPaths(const std::vector<VariableWidthLines>& src_toolpaths);

  private:
    const Point getZSeamHint(const Settings& settings, const Point& center_of_mass);
    const Settings& settings;
    const LayerIndex layer_nr;
